...
Die Ableitung einer bestimmten Größe wird durch einen hochgestellten Index wiedergegeben. Somit ist $S^{B_\beta}_{\mu\nu}$ das nach der $\beta$-Komponente des Mangetfelds abgeleitete Überlappungsintegral der beiden Basisfunktionen $\chi_\mu$ und $\chi_\nu$
...
%  	\end{aligned}  
	...
	sind und damit für alle Werte des externen Magnetfelds und der Kernmomente orthonormal sein müssen. Es gilt daher beispielsweise die folgende Orthonormalitätsbedingung
...
\subsection{CPHF-Gleichungen für Abschirmungskonstanten}\label{kap:cphf}
	...
	\sum_{\mu\nu}c_{\mu q}^*\left(F_{\mu\nu}^{B_\beta}c_{\nu i}-\varepsilon_i S_{\mu\nu}^{B_\beta}c_{\nu i}\right)=\sum_{\mu\nu}c_{\mu q}^*\left(\varepsilon_i S_{\mu\nu}c_{\nu i}^{B_\beta}-F_{\mu\nu}c_{\nu i}^{B_\beta}\right).
	\end{equation}
	...
	welcher in Gleichung (\ref{eq:rhdersort}) eingesetzt wird. Zusätzliches Ausnutzen von ursprünglichen Roothaan-Hall-Gleichungen (\ref{roothaanhall}) sowie der Orthornormalitätsbedingung Gleichung (\ref{eq:orthonormal}) führt damit zu
	
    \begin{equation}\label{eq:cphfupi}
    \begin{aligned}
	\sum_{\mu\nu}c_{\mu q}^*\left(F_{\mu\nu}^{B_\beta}c_{\nu i}-\varepsilon_i S_{\mu\nu}^{B_\beta}c_{\nu i}\right)=&\sum_p\sum_{\mu\nu}c_{\mu q}^*\left(\varepsilon_i S_{\mu\nu}c_{\nu p}U_{pi}^{B_\beta}-\varepsilon_p S_{\mu\nu}c_{\nu p}U_{pi}^{B_\beta}\right)\\
	=&\sum_p\left(\left(\varepsilon_i-\varepsilon_p\right)\delta_{pq}U_{pi}^{B_\beta}\right)\\
	=&\left(\varepsilon_i-\varepsilon_q\right)U_{pi}^{B_\beta}.
	\end{aligned}
	\end{equation}	
	...
	=&\sum_p\left(U_{pi}^{B_\beta}\delta_{pj}+U_{pj}^{B_\beta}\delta_{pi}\right)=U_{ji}^{B_\beta}+U_{ij}^{B_\beta}.
	\end{aligned}
	\end{equation}
    ...
    \begin{equation}
    \begin{aligned}
    F_{\mu\nu}^{B_\beta}=&\frac{\partial}{\partial B_\beta} \left(F_{\mu\nu}\right)_{\vec{\mu}_K=\vec{B}=0}=h_{\mu\nu}^{B_\beta}+G_{\mu\nu}^{B_\beta}\\
    ...
    Damit sind die \ac{cphf}-Gleichungen über den Austausch gekoppelt und müssen in einem iterativen Verfahren näherungsweise gelöst werden.
