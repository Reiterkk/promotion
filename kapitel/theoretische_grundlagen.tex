...
In der Praxis werden die Einelektronen-Wellenfunktionen $\phi_i$ in einer finiten Basis entwickelt. Beim sogenannten \aclu*{lcao-}\mbox{(\acs{lcao}-)}\acused{lcao}Ansatz werden die $\phi_i$ durch die Linearkombination der Basisfunktionen $\chi_\mu$ gebildet,
\begin{equation}
...
Mit Kenntnis der Energie und der Wellenfunktion lassen sich nun molekulare Eigenschaften berechnen, welche als Ableitung der Energie formuliert werden können. Im folgenden Abschnitt werden die für diese Arbeit notwendigen Gleichungen zur Berechnung von Abschirmungskonstanten hergeleitet.    
...
%  	\end{aligned}  
     ...
     \end{aligned}
  	 ...
  	 \end{aligned}
  	 \end{equation}
  	 aus Referenz \cite{ditchfield1974self} ausgenutzt, welche beispielsweise in Referenz \cite{baron1991} bewiesen wird. Mit der verkürzten Notation% für die abgeleiteten Vierzentren-Zweielektronen-Integrale
	\begin{equation}
	\begin{aligned}
...
\subsection{CPHF-Gleichungen für Abschirmungskonstanten}\label{kap:cphf}
    ...
    Mithilfe der Koeffizienten lässt sich dann die optimale Kombination der $\left(U_{ai}^{B_\beta}\right)^{(k,\textrm{opt})}$ und $\left(A_{ai}^{B_\beta}\right)^{(k,\textrm{opt})}$ für die aktuelle Iteration aus allen bisher berechneten Matrizen bestimmen, 
...
\subsection{Implementierung der nach den Komponenten des Magnetfeldes abgeleiteten Integrale}
