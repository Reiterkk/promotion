...
\begin{figure}[ht!]
	...
	\captionsetup{figurewithin = chapter}
	\captionsetup{font=small, labelfont=bf}\caption[Simulierte \ac{vcd} Spektren von \ac{cpa}]{Simulierte \ac{vcd} Spektren von \ac{cpa} für die folgenden Funktional und Basissatz Kombinationen: BP86/def2-TZVP, BP86/def2-SV(P), B3LYP/def2-TZVP und B3LYP/def2-SV(P) (a). Zur Verbreiterung der Linien wurden Lorentzkurven verwendet. Die Halbwertsbreite beträgt \unit[4]{cm$^{-1}$}. Das experimentelle Spektrum (b) wurde aus Referenz \cite{brotin2006vibrational} entnommen.}
\label{abb:cpa_vcd}
\end{figure}
...
\captionabove[Rechenzeiten für \ac{vcd} Spektren]{Rechenzeiten in Stunden für Ir(ppy)$_3$ und \ac{cpa} für unterschiedliche Einstellungen der Rechenparameter. $N_{\textrm{CPU}}$ entspricht der Anzahl an verwendeten CPUs für den \texttt{aoforce} Schritt. Die Berechnungen wurden auf einem Intel\textregistered Xeon\textregistered Prozessor E5-2687W v2, 3.4 GHz durchgeführt.}
\resizebox{\textwidth}{!}{%
\begin{tabular}{ccccccccc}
\hline \hline
Funktional & Basis & $N_{\textrm{BF}}$ & Symmetrie & $N_{\textrm{CPU}}$ & Wall Zeit & \texttt{mpshift} & \texttt{aoforce} & \texttt{aoforce} \\ 
 & & & & & & (CPU) & (CPU) & (wall)\\
\hline
 & & & & & & & & \\
\multicolumn{9}{c}{Ir(ppy)$_3$, ppy=2-Phenylpyridin}\\
...
\multicolumn{9}{c}{Cryptophane-A}\\
B3LYP & TZVP & 2370 & $D_3$ & 4 & 92.1 & 2.58 & 352.70 & 89.47\\
...
\begin{figure}[ht!]
	\centering
	\includegraphics[width=0.75\textwidth]{carboncluster_vcd}
	\captionsetup{figurewithin = chapter}
